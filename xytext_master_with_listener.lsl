...
// Bank configuration - SET THIS FOR EACH OBJECT
integer MY_BANK = 1; // Change this: 0 for left, 1 for middle, 2 for right
...
PassToRender(integer render_type, string message, integer bank) {
    ...
    llOwnerSay("DEBUG: Bank " + (string)MY_BANK + " received message: " + message);
...
init()
{
    ...
    llOwnerSay("DEBUG: Scanning " + (string)num_prims + " prims for bank " + (string)MY_BANK);
    
    for(x=0;x<=num_prims;++x)
    {
        link_name=llGetLinkName(x);
        llOwnerSay("DEBUG: Prim " + (string)x + " name: '" + link_name + "'");
        ...
        if(llList2String(tmp,0) == "xyzzytext")
        {
            ...
            llOwnerSay("DEBUG: Found xyzzytext prim - bank: " + (string)prim_bank + ", position: " + (string)prim_position);
            
            if (prim_bank == MY_BANK)
            {
                llOwnerSay("DEBUG: Adding prim " + (string)x + " to bank " + (string)MY_BANK);
    ...
    if (temp_bank != [])
    {
        llOwnerSay("DEBUG: Found " + (string)(llGetListLength(temp_bank)/temp_bank_stride) + " prims for bank " + (string)MY_BANK);
        ...
        for (x = 0; x < temp_len; x += temp_bank_stride)
        {
            ...
            llOwnerSay("DEBUG: Packing prim " + (string)link_num + " at position " + (string)prims_pointer);
            gXyTextPrims = pack_and_insert(gXyTextPrims, prims_pointer, link_num);
            ++prims_pointer;
        }
    } else {
        llOwnerSay("DEBUG: No prims found for bank " + (string)MY_BANK);
    }
    
    llOwnerSay("XyzzyText initialized for bank " + (string)MY_BANK + " with " + (string)prims_pointer + " prims");
}
...
default { 
   state_entry() { 
       ...
       llOwnerSay("XyzzyText Bank " + (string)MY_BANK + " ready - listening on channel " + (string)LEADERBOARD_CHANNEL);
   ...
   listen(integer channel, string name, key id, string message) {
        if (channel == LEADERBOARD_CHANNEL) {
            llOwnerSay("DEBUG: Bank " + (string)MY_BANK + " heard: " + message);
            ...
            if (MY_BANK == 0 && llSubStringIndex(message, "LEFT_TEXT|") == 0) {
                ...
                llOwnerSay("DEBUG: Bank 0 processing LEFT text: " + leftText);
                ...
                llOwnerSay("DEBUG: Bank 1 processing MIDDLE text: " + middleText);
                ...
                llOwnerSay("DEBUG: Bank 2 processing RIGHT text: " + rightText);
                PassToRender(1, rightText, 2);
                
            } else if (message == "CLEAR_LEADERBOARD") {
                // Clear display for this bank
                llOwnerSay("DEBUG: Bank " + (string)MY_BANK + " clearing display");
